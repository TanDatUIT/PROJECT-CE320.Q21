...
def load_existing_latest_capture() -> None:
    ...
    pc_result: dict[str, object] = {}
    try:
        with (CAPTURE_DIR / filename).open("rb") as fimg:
            pc_result = pc_fuzzy.analyze_jpeg(fimg.read()).to_dict()
            ...
            with ser:
                ...
                while True:
                    ...
                    if line == "CAPTURE_JPG_BASE64_END" and pending_kind != "idle":
                        ...
                        try:
                            conf_pct = int(round(float(conf_tag) * 100))
                        except (TypeError, ValueError):
                            conf_pct = 0
                        filename = f"capture_{esp_time}_{gesture_tag}_{conf_pct:02d}.jpg"
                        ...
                        append_csv(capture)
                        STATE.add_line(
                            f"[PC] Saved {filename} | PC_FUZZY gesture={pc_result.get('gesture')} "
                            f"conf={pc_result.get('confidence')} reason={pc_result.get('reason')}"
                        )
                        print(f"[PC] Saved {path} pc={pc_result.get('gesture')}", flush=True)
...
<div class="metrics">
<div class="metric" style="background:#e6f4ff;border:2px solid #1a73e8"><div class="label">PC Fuzzy (final)</div><div class="value" id="pcGesture">-</div><div class="small" id="pcReason">-</div></div>
<div class="metric"><div class="label">PC Confidence</div><div class="value" id="pcConfidence">-</div></div>
<div class="metric"><div class="label">ESP Hand</div><div class="value" id="hand">-</div></div>
<div class="metric"><div class="label">ESP Gesture</div><div class="value" id="fingers">-</div></div>
...
<div class="controls">
<div><div class="label">Capture Interval ms (0=off, 10000=10s, 30000=30s)</div><input id="capture_ms" type="range" min="0" max="60000" step="1000" value="10000"></div>
...
async function refresh(){
  try {
    ...
    if(c){
      ...
      var pcf = c.pc_fuzzy || {};
      var pcG = pcf.gesture || '-';
      document.getElementById('pcGesture').textContent = pcG.toUpperCase();
      document.getElementById('pcGesture').style.color = pcG==='open' ? '#0a7' : (pcG==='fist' ? '#c33' : '#666');
      document.getElementById('pcConfidence').textContent = Number(pcf.confidence || 0).toFixed(2) + ' | peaks=' + (pcf.finger_peaks!=null?pcf.finger_peaks:'-') + ' edge=' + (pcf.touches_edge?'Y':'N');
      document.getElementById('pcReason').textContent = pcf.reason || '-';
