...
constexpr bool kAlwaysDumpCaptureBase64 = false;
...
void loop() {
  ...
  if (shouldAnalyze) {
    ...
    if (shouldCapture) {
      ...
      if (saveCaptureImage(fb, gMetrics, nowMs, capturePath, sizeof(capturePath))) {
        Serial.printf("CAPTURE_FILE,%s\n", capturePath);
      }
      printCaptureLog(gMetrics, nowMs, capturePath);
